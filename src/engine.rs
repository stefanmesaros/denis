...
pub struct CollectorConfig {
    ...
    /// Any number of extra, capture-only interfaces (switch mirror/SPAN destination ports, one
...
pub struct StatusInfo {
    ...
    /// Any extra, mirror/SPAN capture-only interfaces, if configured and open.
    pub mirror_interfaces: Vec<String>,
    pub ip: Ipv4Addr,
    pub mac: Mac,
    pub subnet: String,
...
struct Collector {
    ...
    /// Any extra, capture-only interfaces (mirror/SPAN ports), if configured.
    mirror_captures: Vec<CaptureThread>,
...
impl Collector {
    async fn start(cfg: &CollectorConfig, store: Arc<dyn Store>, mode: &'static str, cli_retention_days: i64) -> Result<Collector> {
        ...
        // address of its own, so each is validated and opened by name alone.
        // Duplicates (and the main interface itself) are silently collapsed:
        // opening the same interface twice would just double-count everything.
        let mut mirror_names: Vec<&String> = cfg.mirror_ifaces.iter().filter(|n| n.as_str() != iface.name).collect();
        mirror_names.sort();
        mirror_names.dedup();
        let mut mirror_caps = Vec::new();
        ...
        let shared = Arc::new(Shared {
            info: Mutex::new(StatusInfo {
                ...
                mirror_interfaces: mirror_caps.iter().map(|(n, _)| n.clone()).collect(),
                ip: iface.ip,
                mac: iface.mac,
