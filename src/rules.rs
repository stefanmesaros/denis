...
pub static PARAMS: &[Param] = &[
    ...
    param("new_port_churn_max", "Stop repeating after", "alerts to the same destination", "Some conversations (a P2P/relay service that hands out a fresh port per session) never settle down; after this many new-port alerts to the same destination, DENIS stops raising more.", 1.0, 50.0, 1.0,
        |c| c.port_churn_max as f64, |c, v| c.port_churn_max = v as u32),
    param("dest_port_churn_max", "Stop repeating after", "alerts on the same service (NTP, STUN)", "NTP and STUN talk to a different server address every time by design; after this many new-destination alerts on that service, DENIS stops raising more.", 1.0, 50.0, 1.0,
        |c| c.dest_port_churn_max as f64, |c, v| c.dest_port_churn_max = v as u32),
...
pub static RULE_INFO: &[RuleInfo] = &[
    ...
    RuleInfo { id: "new_destination", title: "New destination", group: "network",
        ...
        needs: "traffic analysis (--flows)", params: &["dest_port_churn_max"] },
    RuleInfo { id: "new_port", title: "New service port", group: "network",
        ...
        needs: "traffic analysis (--flows)", params: &["new_port_min_known", "new_port_churn_max"] },
