...
step "1/9 the version is consistent (Cargo.toml, CHANGELOG.md)"
...
step "2/9 unit and integration tests"
...
step "3/9 lint (warnings are errors)"
cargo clippy --locked --all-targets -- -D warnings

step "4/9 translations are complete and the generated catalogs are current"
...
step "5/9 the installer (syntax, and a dry run that downloads and verifies the newest release)"
...
step "6/9 build the release program"
cargo build --release --locked

step "7/9 the console in a real browser (every page, the asset editor, icons, rules, all languages, phone layout)"
DENIS_BIN=target/release/denis node tools/ui-smoke.mjs

step "8/9 a database from the newest published release opens with this program"
if [ "${OFFLINE:-0}" = 0 ]; then tools/upgrade-check.sh target/release/denis; else echo "(offline: skipped)"; fi

step "9/9 the industrial decoders and OT rules on real public captures, and on encrypted traffic"
if [ "${OFFLINE:-0}" = 0 ]; then DENIS_BIN=target/release/denis tools/ot-samples.sh; else OFFLINE=1 DENIS_BIN=target/release/denis tools/ot-samples.sh; fi
