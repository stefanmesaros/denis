...
# something for the Reports page to show: one report, made now, and a weekly schedule
def post(path, body=None):
    r = urllib.request.Request(base + path, method="POST", headers={"X-Denis": "1", "Content-Type": "application/json"}, data=json.dumps(body or {}).encode())
    return json.load(urllib.request.urlopen(r))
if not json.load(urllib.request.urlopen(base + "/api/reports"))["reports"]:
    post("/api/reports", {"days": 7})
